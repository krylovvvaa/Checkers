...
private:
	...
	Board board;
	//RectangleShape framing = RectangleShape(Vector2f(460, 460));
public:
	Board &get_board() {
		return this->board;
	}
	...
	void start_game() {
		for (int i = 0; i < 24; i++) {//шашки
			if (i < 4) {
				checkers.push_back(Checker((float)i * 100 + 102, (float)88, 1));
				board.get_all_squares(i * 2 + 1, 0).square_employment(1);
			}
			else if (i >= 4 && i < 8) {
				checkers.push_back(Checker((float)(i - 4) * 100 + 52, (float)138, 1));
				board.get_all_squares((i - 4) * 2, 1).square_employment(1);
			}
			else if (i >= 8 && i < 12) {
				checkers.push_back(Checker((float)(i - 8) * 100 + 102, (float)188, 1));
				board.get_all_squares((i - 8) * 2 + 1, 2).square_employment(1);
			}

			else if (i >= 12 && i < 16) {
				checkers.push_back(Checker((float)(i - 12) * 100 + 52, (float)338, 0));
				board.get_all_squares((i - 12) * 2, 5).square_employment(0);
			}
			else if (i >= 16 && i < 20) {
				checkers.push_back(Checker((float)(i - 16) * 100 + 102, (float)388, 0));
				board.get_all_squares((i - 16) * 2 + 1, 6).square_employment(0);
			}
			else if (i >= 20 && i < 24) {
				checkers.push_back(Checker((float)(i - 20) * 100 + 52, (float)438, 0));
